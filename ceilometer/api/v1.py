...
def compute_project_volume_max(project, meter):
    ...
    # by resource_id.  We should add a new function storage driver that
    # does not do this grouping (and potentially rename the existing one
    # to get_volume_max_by_resource())
    results = list(flask.request.storage_conn.get_volume_max(f))
    value = None
    if results:
...
def compute_project_volume_sum(project, meter):
    ...
    # FIXME(sberler): Currently get_volume_sum is really always grouping
    # by resource_id.  We should add a new function storage driver that
    # does not do this grouping (and potentially rename the existing one
    # to get_volume_sum_by_resource())
    results = list(flask.request.storage_conn.get_volume_sum(f))
    value = None
    if results:
