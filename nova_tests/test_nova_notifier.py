...
class TestNovaNotifier(base.TestCase):
    ...
    def setUp(self):
        ...
        # plugin
        self.notifications = []
        notifier_api._drivers = {
            nova_notifier.__name__: nova_notifier,
            'test': self,
        }
        ...
        gatherer = nova_notifier.DeletedInstanceStatsGatherer(ext_mgr)
        nova_notifier.initialize_gatherer(gatherer)
    ...
    def fake_db_instance_get(self, context, id_):
        if self.instance['uuid'] == id_:
            i = mock.MagicMock(name=self.instance['name'],
                               id=self.instance['uuid'])
            setattr(i, 'OS-EXT-SRV-ATTR:instance_name', self.instance['name'])
            return i
    ...
    def test_notifications(self):
        ...
        # Ensure the pollster was called as we expect
        self.assertTrue(self.Pollster.instances)
        self.assertTrue(self.Pollster.instances[0])
        self.assertEqual(self.Pollster.instances[0][0],
                         nova_notifier._gatherer)
        self.assertEqual(self.Pollster.instances[0][1].id,
                         self.instance['uuid'])
